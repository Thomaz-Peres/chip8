...
public sealed class program
{
    public static void Main(string[] args)
    {
        ...
        int videoPitch = sizeof(uint) * 64;

        var cycleDelay = Convert.ToInt32(args[1]);
        var sw = Stopwatch.StartNew();
        var lastCycleTime = sw.ElapsedMilliseconds;
        bool quit = false;
        ...
        while (!quit)
        {
            ...
            var ct = sw.ElapsedMilliseconds;
            var dt = ct - lastCycleTime;
            ...
            if (dt > cycleDelay)
            {
                ...
                unsafe
                {
                    fixed (uint* videoPtr = cpu.GetVideoPtr())
                    {
                        sdlManager.Update(videoPtr, videoPitch);
                    }
                }
            }
        }
